...
xyFname = 'PosXY.txt'
ltFname = 'LightTable.txt'

load('Params.mat');
TimeTable = readtable(ttFname);
PosXY = readmatrix(xyFname);
LightTable = readtable(ltFname);
LightTable.currTime = TimeTable.currTime;
%% Filter data 
...
AnalysisParams.ActivityBinLength = 1% str2num(answer{3});
...
clearvars -except AnalysisParams Params TimeTable LightTable PosXY FData saveDir
% TineWrapPlots
% [Xs,Ys] = TimeWrapXY(ZT,FData.ShadeSleepPI,24)
% figure
% binLength = 30
% ZT = (0:AnalysisParams.Length*24/(AnalysisParams.Length*1440/binLength):(AnalysisParams.Length*24) - ((AnalysisParams.Length*24)/1440/binLength))'
% hold on
...
%get results table 
light_pulse_table
clearvars -except AnalysisParams Params TimeTable LightTable PosXY FData ArousalTable saveDir
